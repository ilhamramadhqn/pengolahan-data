        ...
        <div class="col-md-12">
          <div class="card">
            ...
            <div class="card-body p-0">
              <form method="post" action="{{ route('Penelitian.update', $data->id_penelitian) }}" enctype="multipart/form-data">
                @csrf
                @method('PATCH')
                <div class="card-body">
                  <div class="form-group">
                    <label>Silahkan Masukan Judul Penelitian</label>
                    <input type="text" name="judul_penelitian" id="judul_penelitian" class="form-control" value="{{$data->judul_penelitian}}"onkeyup="this.value = this.value.toUpperCase();" autofocus>
                  ...
                  <select name="id_jenis_penelitian" id="id_jenis_penelitian" class="form-control">
                    @foreach($jenispublikasi as $jp)  
                      <option value="{{$jp->id_jenis_penelitian}}" @if($jp->id_jenis_penelitian == $data->id_jenis_penelitian){{ 'selected' }} @endif>{{$jp->nama_jenis_penelitian}}</option>
                    @endforeach
                  </select>
                  </div>
                  <div class="form-group">
