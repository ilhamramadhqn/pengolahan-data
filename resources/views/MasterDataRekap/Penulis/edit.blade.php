        ...
        <div class="col-md-12">
          <div class="card">
            <div class="card-header">
              <h4>Edit Data Penulis</h4>
            </div>
            <div class="card-body p-0">
              <form method="post" action="{{ route('Penulis.update', $data->id_penulis) }}">
                @csrf
                @method('PATCH')
                <div class="card-body">
                  ...
                  <div class="form-group">
                    <label>Silahkan Masukan Artikel</label>
                    <select name="id_artikel" id="id_artikel" class="form-control">
                    @foreach($artikel as $a)  
                      <option value="{{$a->id_artikel}}" @if($a->id_artikel == $data->id_artikel){{ 'selected' }} @endif>{{$a->judul_artikel}}</option>
                    @endforeach
                    </select>
                  </div>
                  <div class="form-group">
                ...
                <div class="card-footer text-right">
                  <a href="/Penulis" class="btn btn-danger">Back</a>
