        ...
        <div class="col-md-12">
          <div class="card">
            <div class="card-header">
              <h4>Edit Data Pencipta</h4>
            </div>
            <div class="card-body p-0">
              <form method="post" action="{{ route('Pencipta.update', $data->id_pencipta) }}">
                @csrf
                @method('PATCH')
                <div class="card-body">
                  ...
                  <div class="form-group">
                    ...
                    @foreach($hki as $h)  
                      <option value="{{$h->id_hki}}" @if($h->id_hki == $data->id_hki){{ 'selected' }} @endif>{{$h->judul_hki}}</option>
                    @endforeach
                    </select>
                  </div>
                  <div class="form-group">
